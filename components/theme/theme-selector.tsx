...
const THEMES: Theme[] = [
  ...
  },
  {
    ...
    unlockRequirement: { type: "xp", value: 250 },
    isUnlocked: false,
    isActive: false
  },
  {
  ...
  },
  {
    ...
    unlockRequirement: { type: "rank", value: "Diamond" },
    isUnlocked: false,
    isActive: false
  },
  {
...
export function ThemeSelector({ userXP, userRank, isPremium, currentTheme }: ThemeSelectorProps) {
  ...
  useEffect(() => {
    ...
    const updatedThemes = THEMES.map(theme => {
      ...
      switch (theme.unlockRequirement.type) {
        case "xp":
          isUnlocked = userXP >= (theme.unlockRequirement.value as number)
          break
        case "rank":
      ...
      return {
        ...theme,
        isUnlocked: theme.id === "default" ? true : isUnlocked,
        isActive: theme.id === activeTheme
      }
    })

    setThemes(updatedThemes)
  }, [userXP, userRank, isPremium, activeTheme])
  ...
  const getUnlockText = (theme: Theme) => {
    switch (theme.unlockRequirement.type) {
      case "xp":
        return `${theme.unlockRequirement.value} XP required`
      case "rank":
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Unlock themes by earning XP, achieving ranks, or upgrading to Premium.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="pt-4 border-t border-border/30">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-bold text-foreground">{userXP.toLocaleString()}</span>
            </div>
